"""
Management command to test the statistics tracking system.
"""
...
class Command(BaseCommand):
    help = 'Test the statistics tracking system by creating sample data and verifying queries'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--aggregate',
            action='store_true',
            help='Run statistics aggregation',
        )
        parser.add_argument(
            '--verify',
            action='store_true',
    ...
    def handle(self, *args, **options):
        ...
        if not any([options['create_sample'], options['aggregate'], options['verify']]):
    ...
    def run_aggregation(self):
        ...
        self.stdout.write("Running statistics aggregation...")
        
        try:
            StatisticsAggregator.update_all_statistics()
            self.stdout.write(self.style.SUCCESS('✓ Statistics aggregation completed'))
        except Exception as e:
            self.stdout.write(self.style.ERROR(f'✗ Aggregation failed: {e}'))
            raise

    def verify_system(self):
        """Verify that the statistics system is working correctly"""
        self.stdout.write("\n" + "="*60)
        ...
        self.stdout.write(f"GameSession: {total_games} total, {valid_games} valid games")
        ...
        # Check ServerStatistics
        total_server_stats = ServerStatistics.objects.count()
        self.stdout.write(f"ServerStatistics: {total_server_stats} servers tracked")
        
        if total_server_stats > 0:
            top_server = ServerStatistics.objects.order_by('-total_valid_games').first()
            self.stdout.write(f"  Most Active: {top_server.server_name}")
            self.stdout.write(f"  Valid Games: {top_server.total_valid_games}")
            self.stdout.write(f"  Avg Players: {top_server.avg_players_per_game:.1f}")
        else:
            self.stdout.write(self.style.WARNING("⚠ No server statistics found. Run with --aggregate to compute."))
        
        self.stdout.write("")
        
        # Check MapStatistics
        total_map_stats = MapStatistics.objects.count()
        self.stdout.write(f"MapStatistics: {total_map_stats} maps tracked")
        
        if total_map_stats > 0:
            top_map = MapStatistics.objects.order_by('-total_valid_games').first()
            self.stdout.write(f"  Most Played: {top_map.map_name}")
            self.stdout.write(f"  Valid Games: {top_map.total_valid_games}")
            self.stdout.write(f"  Avg Duration: {int(top_map.avg_game_duration // 60)}m")
        else:
            self.stdout.write(self.style.WARNING("⚠ No map statistics found. Run with --aggregate to compute."))
        ...
        elif total_games > 0 and valid_games == 0:
