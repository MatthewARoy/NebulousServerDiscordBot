...
matplotlib is intentionally NOT imported at module load. It pulls in numpy
transitively and adds ~80-120 MiB to the bot's RSS, which is the difference
between "comfortable" and "swap-thrashing" on the 503 MiB Always-Free VM.
The two methods that actually render images do the import on first call.
"""
...
def _load_matplotlib():
    """Import matplotlib + pyplot + dates with the Agg backend pinned.

    Called lazily from the rendering methods so the bot can start (and live
    its whole life, if !graph is never used) without these in RSS. Subsequent
    calls are cheap — Python caches the import.
    """
    import matplotlib
    matplotlib.use('Agg')  # Non-interactive; safe to call repeatedly
